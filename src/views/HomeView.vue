  ...
  methods: {
    populate() {
      ...
      for (let i = -periode; i < periode; i++) {
        ...
        let t_stamp = now + i
        table.push({ i: i, name: "j" + i, date: d.toLocaleString(), t_stamp: t_stamp, x: d.getDate(), y: d.getMonth() })
      }
    ...
    init() {
      ...
      for (let i = 0; i < table.length; i++) {
        ...
        element.style.backgroundColor = time.i == 0 ? 'rgba(127,0,0,' + (Math.random() * 0.5 + 0.25) + ')' : 'rgba(0,127,127,' + (Math.random() * 0.5 + 0.25) + ')';

        const number = document.createElement('div');
        number.className = 'number';
        number.textContent = time.date;
        element.appendChild(number);

        const symbol = document.createElement('div');
        symbol.className = 'symbol';
        symbol.textContent = time.name;
        element.appendChild(symbol);

        const details = document.createElement('div');
        details.className = 'details';
        details.innerHTML = "details"//time.id + '<br>' + time.name;
        ...
        const objectCSS = new CSS3DObject( element );
					...
					object.position.x = ( time.x  * 140 ) - 1330;
					object.position.y =  - ( time.y * 180 ) + 990;
